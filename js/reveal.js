...
var Reveal = (function(){
	...
	function initialize( options ) {
		...
		dom.outline = document.querySelector( '.reveal .outline' );
		...
		if ( config.outline ){
			...
			var childNodes = document.querySelector('div.slides').children;
			var outlineNode = document.createElement('span');
			var node, title;
			var verticalNodes;
			for(var i = 0, len = childNodes.length; i < len; i++  ) {
				...
				if (node.nodeName == 'ARTICLE') { 
					...
					var outline_item = outlineNode.cloneNode(true);
					outline_item.appendChild(document.createTextNode(title));
					dom.outline.appendChild(outline_item);
				} else if (node.nodeName == 'SECTION' && current_article >= 0 && !node.classList.contains('no-outline') )
				{
					node.setAttribute('data-outline', current_article);
					if (verticalNodes = node.querySelectorAll('section')) {
						for (var j =0, nlen = verticalNodes.length; j < nlen; j++) {
							verticalNodes[j].setAttribute('data-outline', current_article);
						}
					}
				}
			}
